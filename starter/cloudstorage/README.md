...
3. **Password Management:** Save, edit, and delete website credentialData.  
...
1. Managing user access with Spring Security
 ...
 - You also need to implement a custom `AuthenticationProvider` which authorizes user logins by matching their credentialData against those stored in the database.  
...
The home page is the center of the application and hosts the three required pieces of functionality. The existing template presents them as three tabs that can be clicked through by the user:
 ...
 iii. Credentials
 - The user should be able to store credentialData for specific websites and see a list of the credentialData they've previously stored. If you display passwords in this list, make sure they're encrypted!
 - The user should be able to view/edit or delete individual credentialData. When the user views the credential, they should be able to see the unencrypted password.
...
3. Write tests for credential creation, viewing, editing, and deletion.
 - Write a test that creates a set of credentialData, verifies that they are displayed, and verifies that the displayed password is encrypted.
 - Write a test that views an existing set of credentialData, verifies that the viewable password is unencrypted, edits the credentialData, and verifies that the changes are displayed.
 - Write a test that deletes an existing set of credentialData and verifies that the credentialData are no longer displayed.

## Final Tips and Tricks
### Password Security
Make sure not to save the plain text credentialData of the application's users in the database. That's a recipe for data breach disaster! Use a hashing function to store a scrambled version instead. Your tech lead gave you a class called `HashService` that can hash passwords for you. When the user signs up, you only store a hashed version of their password in the database, and on login, you hash the password attempt before comparing it with the hashed password in the database. Your tech lead knows that can be a little confusing, so they provided this code sample to help illustrate the idea:
...
For storing credentialData in the main part of the application, we can't hash passwords because it's a one-way operation. The user needs access to the unhashed password, after all! So instead, you should encrypt the passwords. Your tech lead provided you with a class called `EncryptionService` that can encrypt and decrypt passwords. When a user adds new credentialData, encrypt the password before storing it in the database. When the user views those credentialData, decrypt the password before displaying it. Here's a little code snippet on how to use `EncryptionService`:
